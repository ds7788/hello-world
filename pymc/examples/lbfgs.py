...
with model:
    s,scale = guess_starting_scaling(n = 300, na = 0)
    step = NUTS(scaling = scale)
    trace = sample(300, step, s)
...
for i, country in enumerate(countries):
    ...
    d = data[data.area == country]
    plot(d.age, d.value, '.')
    plot(knots, trace[y][::,:,i].T, color ='r', alpha =.01);
...
plot(step.potential.velocity(step.potential.random()))

# <codecell>

step.potential.lbfgs.grads.n
